...
document.getElementById('form-ts')?.addEventListener('submit', function (e) {
  ...
  if (!sheetRow) {
    const isDuplicate = rawTs.some(r => String(r.nisn).trim() === String(parts[0]).trim() && String(r.pembayaran).trim().toLowerCase() === finalPembayaran.toLowerCase());
    if (isDuplicate) {
      alert("Sudah Bayar! Transaksi " + finalPembayaran + " untuk NISN " + (parts[0] || '') + " sudah dicatat sebelumnya.");
