      ...
      listTarif.forEach(tarif => {
        ...
        listTransaksi.forEach(trx => {
          if (String(trx.nisn).trim() === String(siswa.idLogin).trim()) {
            ...
            } else if (trx.pembayaran === tarif.namaPembayaran + ' (Beasiswa)') {
              isBeasiswa = true;
              terbayar += parseFloat(trx.nominal) || 0;
            }
          }
...
function loadTagihanSiswa() {
  ...
  Promise.all([
    ...
    const listTransaksi = (resTransaksi.status === 'success') ? resTransaksi.data.filter(t => String(t.nisn).trim() === String(nisn).trim()) : [];
