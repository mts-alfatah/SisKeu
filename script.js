...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (menuToggle && sidebar) {
    ...
    // Menutup menu jika klik di luar area sidebar (area main-content)
...
// ===================================================
// 1. LOGIKA HALAMAN LOGIN (Akan berjalan jika ada form login)
// ===================================================
...
// Pengecekan Akses Sesi Dashboard
if (pageTitleElem) {
...
function clearFilter(prefix) {
  ['search-', 'from-', 'to-', 'jenis-', 'role-'].forEach(p => {
...
function loadLaporanTotal() {
  Promise.all([
    ...
    listTs.forEach(item => {
      ...
      totalPemasukan += nom;
      rawLaporan.push({ tanggal: item.tanggal, sumber: 'Siswa (' + item.nama + ')', keterangan: item.pembayaran, jenis: 'Pemasukan', nominal: nom, admin: item.admin });
    });
    ...
    listTi.forEach(item => {
      ...
      rawLaporan.push({ tanggal: item.tanggal, sumber: 'Internal', keterangan: item.keterangan, jenis: item.jenis, nominal: nom, admin: item.admin });
    });
...
function renderLaporan() {
  ...
  if (filteredLaporan.length > 0) {
    ...
    tbody.innerHTML = '<tr><td colspan="6" class="empty-state">Tidak ada data laporan untuk periode ini.</td></tr>';
  }
}
...
function exportLaporanExcel() {
  ...
  const dataForExcel = filteredLaporan.map((r, idx) => ({ 'No': idx + 1, 'Tanggal': r.tanggal, 'Sumber': r.sumber, 'Keterangan': r.keterangan, 'Jenis': r.jenis, 'Nominal (Rp)': r.nominal, 'Petugas': r.admin || '-' }));
...
function exportLaporanPDF() {
  ...
  const tableRows = filteredLaporan.map((r, idx) => [idx + 1, r.tanggal, r.sumber, r.keterangan, r.jenis, formatRp(r.nominal), r.admin || '-']);
  doc.autoTable({ startY: 28, head: [['No', 'Tanggal', 'Sumber', 'Keterangan', 'Jenis', 'Nominal', 'Petugas']], body: tableRows, theme: 'grid', headStyles: { fillColor: [13, 110, 253] } });
...
document.getElementById('form-ts')?.addEventListener('submit', function(e) {
  ...
  fetch(scriptURL, {
    ...
    body: JSON.stringify({
      ...
      sheetRow: sheetRow, nisn: parts[0], namaSiswa: parts[1],
  ...
  .then(r => r.json())
  .then(res => { alert(res.message || 'Berhasil disimpan!'); closeModal('modal-transaksi-siswa'); loadTransaksiSiswa(); })
...
document.getElementById('form-ti')?.addEventListener('submit', function(e) {
  ...
  .then(r => r.json())
  .then(res => { alert(res.message || 'Berhasil disimpan!'); closeModal('modal-transaksi-internal'); loadTransaksiInternal(); })
  .catch(err => alert('Gagal menyimpan: ' + err.message))
  .finally(() => { btn.disabled = false; btn.textContent = 'Simpan'; });
});
...
document.getElementById('form-tp')?.addEventListener('submit', function(e) {
  ...
  .then(r => r.json())
  .then(res => { alert(res.message || 'Berhasil disimpan!'); closeModal('modal-jenis-pembayaran'); loadTarifPembayaran(); })
  .catch(err => alert('Gagal menyimpan: ' + err.message))
  .finally(() => { btn.disabled = false; btn.textContent = 'Simpan'; });
});
...
document.getElementById('form-akun')?.addEventListener('submit', function(e) {
  ...
  .then(r => r.json())
  .then(res => {
    if (res.status === 'success') { alert(res.message || 'Berhasil disimpan!'); closeModal('modal-akun'); loadAkun(); }
...
// Pemuatan Awal Data saat Halaman Ditinggalkan/Di-load
