      ...
      checkboxContainer.innerHTML = filteredTarif.map((t, idx) => `
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; border-bottom: 1px solid #eee; padding-bottom: 8px;">
          <label style="margin:0; font-weight:normal; display:flex; align-items:center; gap:8px; cursor:pointer;">
            <input type="checkbox" class="ts_multi_check" value="${escapeHtml(t.namaPembayaran)}" data-nominal="${t.nominalTarif}" id="chk_${idx}" style="cursor:pointer;">
            ${escapeHtml(t.namaPembayaran)}
          </label>
          <input type="number" id="nom_${idx}" class="ts_multi_nominal" value="${t.nominalTarif}" style="width: 120px; padding: 4px; border: 1px solid #ccc; border-radius: 4px;" disabled>
        </div>
      `).join('');
