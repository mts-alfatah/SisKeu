...
function loadTagihanWalikelas() {
  ...
  Promise.all([
    ...
    const listTarif = (resTarif.status === 'success') ? resTarif.data.filter(t => {
      const target = String(t.targetKelas || '').toLowerCase();
      return target.includes('semua') || target.includes((kelas || '').toLowerCase());
    }) : [];
    const listTransaksi = (resTransaksi.status === 'success') ? resTransaksi.data.filter(t => t.kelas === kelas) : [];
    ...
    listSiswa.forEach(siswa => {
      listTarif.forEach(tarif => {
        ...
        listTransaksi.forEach(trx => {
          if (trx.nisn === siswa.idLogin && (trx.pembayaran === tarif.namaPembayaran || trx.pembayaran === tarif.namaPembayaran + ' (Beasiswa)')) {
            terbayar += parseFloat(trx.nominal) || 0;
          }
        });
        const tagihan = (parseFloat(tarif.nominalTarif) || 0) - terbayar;
        rawTagihanWk.push({
          nisn: siswa.idLogin,
          nama: siswa.nama,
  ...
  if (filteredTagihanWk.length > 0) {
    tbody.innerHTML = filteredTagihanWk.map(r => `
      <tr>
        ...
        <td><span style="padding:4px 8px; border-radius:4px; font-size:12px; color:#fff; background:${r.status==='Lunas'?'#198754':'#dc3545'};">${r.status}</span></td>
      </tr>
    `).join('');
  } else {
...
function loadTagihanSiswa() {
  ...
  Promise.all([
    fetch(scriptURL + '?action=getTarifPembayaran').then(r => r.json()),
    fetch(scriptURL + '?action=getTransaksiSiswa').then(r => r.json())
  ]).then(([resTarif, resTransaksi]) => {
    const listTarif = (resTarif.status === 'success') ? resTarif.data.filter(t => {
      const target = String(t.targetKelas || '').toLowerCase();
      return target.includes('semua') || target.includes((kelas || '').toLowerCase());
    }) : [];
    const listTransaksi = (resTransaksi.status === 'success') ? resTransaksi.data.filter(t => t.nisn === nisn) : [];

    rawTagihanSiswa = [];
    listTarif.forEach(tarif => {
      let terbayar = 0;
      listTransaksi.forEach(trx => {
        if (trx.pembayaran === tarif.namaPembayaran || trx.pembayaran === tarif.namaPembayaran + ' (Beasiswa)') {
          terbayar += parseFloat(trx.nominal) || 0;
        }
      });
      const tagihan = (parseFloat(tarif.nominalTarif) || 0) - terbayar;
...
function loadRiwayatSiswa() {
  ...
  fetch(scriptURL + '?action=getTransaksiSiswa')
    .then(r => r.json())
    .then(res => { 
      rawRiwayatSiswa = (res.status === 'success') ? res.data.filter(t => t.nisn === nisn) : [];
      renderRiwayatSiswa();
    })
    .catch(() => { tbody.innerHTML = '<tr><td colspan="4" class="empty-state">Gagal memuat data.</td></tr>'; });
}
